...
const emptyScheduleRow = (): VolunteerScheduleRow => ({
  day: "",
  time: "",
  event: "",
  room: "",
...
function parseSchedule(raw: unknown): VolunteerScheduleRow[] {
  ...
  return raw
    ...
    .map((r) => ({
      ...
      notes:
        String((r as VolunteerScheduleRow).notes ?? "").trim() || undefined,
    }));
}
...
export function EventForm({ event }: Props) {
  ...
  const [scheduleRows, setScheduleRows] = useState<VolunteerScheduleRow[]>(
    () =>
      event?.volunteer_schedule?.length
        ? parseSchedule(event.volunteer_schedule)
        : [],
  );
  const formId = useId();
  ...
  const addScheduleRow = () =>
    setScheduleRows((prev) => [
      ...prev,
      { ...emptyScheduleRow(), day: event?.start_date ?? "" },
    ]);
  const removeScheduleRow = (index: number) =>
    setScheduleRows((prev) => prev.filter((_, i) => i !== index));
  const updateScheduleRow = (
    index: number,
    field: keyof VolunteerScheduleRow,
    value: string,
  ) => {
    setScheduleRows((prev) =>
  ...
  return (
    <form action={formAction} className="flex w-full flex-col gap-4">
      ...
      <input
        ...
        value={JSON.stringify(scheduleRows)}
      />
      {state && "error" in state && (
        <p
          className="rounded-lg border-2 border-papa-accent/30 bg-red-50 p-3 text-sm font-medium text-papa-accent"
          role="alert"
        >
          {state.error}
        </p>
      ...
      {state && "ok" in state && state.ok && (
        <p
          className="rounded-lg border-2 border-green-600/30 bg-green-50 p-3 text-sm font-medium text-green-800"
          role="status"
        >
      ...
      <div>
        ...
        <input
          ...
          name="location"
          type="text"
          placeholder="e.g. TBD, venue name, or full address"
      ...
      <div className="rounded-lg border-2 border-papa-form-border bg-papa-card/80 p-4">
        ...
        <p className="mb-2 text-xs text-papa-muted">
          Add rows for setup, meals, and sessions so volunteers see a quick
          table. Different rooms and times per row are fine.
        </p>
            ...
            scheduleRows.map((row, index) => (
              ...
              >
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-medium text-papa-muted">
                    Day
                  </label>
                  <input
                    type="date"
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    value={row.notes ?? ""}
                    onChange={(e) =>
                      updateScheduleRow(index, "notes", e.target.value)
                    }
                    className="papa-form-input min-w-0 rounded px-2 py-1.5 text-sm"
                  />
                </div>
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-papa-muted">
          Only people who have signed up for this event see the schedule and
          this text. Update whenever you get new details.
        </p>
      </div>
      <div>
        <label
