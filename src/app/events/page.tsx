  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <section className="relative -mx-4 h-[min(45vh,340px)] min-h-[240px] overflow-hidden md:-mx-6 lg:-mx-8">
        ...
        <img
          src="/images/fapa-job-fair.png"
          alt="FAPA Job Fair Dallas 2024"
      ...
      <section className="mt-8 flex flex-col gap-6">
        {!events?.length ? (
          ...
          <ul className="flex flex-col gap-4">
            {(events ?? []).map((event) => {
              const counts = countByEvent[event.id] ?? {
                confirmed: 0,
                waitlist: 0,
              };
              const full = counts.confirmed >= event.capacity;
              const tz =
                TIMEZONE_LABELS[event.timezone as Timezone] ?? event.timezone;
              const timeStr =
                [
                  formatTimeLocal(event.start_time),
                  formatTimeLocal(event.end_time),
                ]
                  .filter(Boolean)
                  .join(" – ") || null;
              return (
